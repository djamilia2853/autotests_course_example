...
def everything_for_your_cat(cats_data):
    """
       Функция приводит в порядок список владельцев и их котов
       :param cats_data: список кортежей с данными о котах и покупателях
       :return: оптимизированная строка в виде ИФ владельца: кот, возраст; кот, возраст
       """

    new_dict = {}
    for item in cats_data:
        cat = item[0] + ', ' + str(item[1])
        new_dict.setdefault(item[2:], []).append(cat)

    our_str = ''
    for fio, cat in new_dict.items():
        our_str += f"{' '.join(fio)}: {'; '.join(cat)}\n"
    return our_str
